...
export default function AboutUsPage() {
  return (
    <main className={cn('container-px pb-20 pt-32 md:pt-36')}>
      <section className={cn('mx-auto max-w-screen-2xl space-y-16')}>
        <div className={cn('grid items-center gap-10 lg:grid-cols-2')}>
          ...
          <AnimatedSection animation='slideLeft' duration='slow'>
            <div
              className={cn(
                'group relative aspect-video overflow-hidden rounded-xl bg-muted',
              )}
            >
              <div
                className={cn(
                  'absolute inset-0 bg-linear-to-br from-primary/25 to-accent/25',
                )}
              />
              <div
                className={cn(
              ...
              >
                <div>
                  <span
                    className={cn(
                      'mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-white/15 backdrop-blur-sm transition-transform duration-300 group-hover:scale-110',
                    )}
                  >
                    <Play
                      className={cn('ml-1 h-8 w-8 fill-white text-white')}
                    />
                  </span>
        ...
        <AnimatedSection animation='slideUp' duration='slow'>
          ...
          >
            <div className={cn('container-px py-12 md:py-16')}>
              <div
                className={cn(
                  'mx-auto grid max-w-screen-2xl grid-cols-1 gap-4 md:grid-cols-3 md:grid-rows-3',
                )}
              >
                <article
                  className={cn(
                    'order-1 relative min-h-52 overflow-hidden rounded-2xl border border-border md:order-0 md:col-start-1 md:row-start-1 md:row-span-2',
                  )}
                >
                  <Image
                ...
                <article
                  className={cn(
                    'order-3 relative min-h-52 overflow-hidden rounded-2xl border border-border md:order-0 md:col-start-2 md:row-start-1 md:row-span-3',
                  )}
                >
                  <Image
                ...
                <article
                  className={cn(
                    'order-4 rounded-2xl border border-border bg-brand-blue/25 p-8 text-left transition-transform duration-300 hover:-translate-y-1 md:order-0 md:col-start-3 md:row-start-1',
                  )}
                >
                ...
                <article
                  className={cn(
                    'order-5 relative min-h-52 overflow-hidden rounded-2xl border border-border md:order-0 md:col-start-3 md:row-start-2 md:row-span-2',
                  )}
                >
                  <Image
                ...
                <article
                  className={cn(
                    'order-2 rounded-2xl border border-border bg-brand-green/25 p-8 text-left transition-transform duration-300 hover:-translate-y-1 md:order-0 md:col-start-1 md:row-start-3',
                  )}
                >
                  <p className={cn(typography.card.statValue, 'font-semibold')}>
                    {stats[0].value}+
        ...
        <AnimatedSection animation='slideUp' duration='normal' delay='short'>
          <div className={cn('space-y-8')}>
            <div
              className={cn(
                'flex flex-col items-start justify-between gap-4 rounded-2xl md:flex-row md:items-center',
              )}
            >
            ...
            <div className={cn('grid gap-4 md:grid-cols-2 xl:grid-cols-3')}>
              {clientReasons.map((reason, index) => (
                <article
                  ...
                  className={cn(
                    'rounded-2xl border border-card bg-card p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-md',
                  )}
                >
                  <h2 className={cn(typography.card.title, 'font-semibold')}>
                    {reason.title}
                  </h2>
